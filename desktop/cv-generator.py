...
class Curriculum:
    ...
    def __init__(self):
        ...
        self.root.option_add('*Entry.background', '#f5f5f7')
    ...
    def setup_gui(self):
        ...
        self.language_button = self.create_button(
            left_frame, self.languages[self.current_language]["language_toggle"], self.toggle_language, 0)
        ...
        # Other UI elements
        self.photo_button = self.create_button(
            left_frame, self.languages[self.current_language]["attach_photo"], self.attach_photo, 7)
    ...
    def create_frame(self, parent, row, col):
        ...
        frame = tk.Frame(
            parent, padx=self.style['padding'], pady=self.style['padding'], bg=self.style['bg_color'])
    ...
    def setup_preview_area(self):
        ...
        self.preview_content = tk.Frame(
            self.preview_canvas, bg=self.style['bg_color'])
    ...
    def create_footer(self):
        ...
        self.coffee_link.bind(
            "<Button-1>", lambda e: webbrowser.open("https://www.paypal.com/paypalme/cripterhack"))
        ...
        github_link.bind(
            "<Button-1>", lambda e: webbrowser.open("https://github.com/CripterHack/cv-generator"))
    ...
    def export_as_markdown(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Éxito", f"CV exportado como Markdown: {filename}")
        except Exception as e:
            messagebox.showerror(
                "Error de exportación", f"Ocurrió un error al exportar el CV como Markdown: {str(e)}")
    ...
    def export_as_pdf(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error de exportación", f"Ocurrió un error al exportar el CV como PDF: {str(e)}")
    ...
    def update_gui_text(self):
        ...
        self.language_button.config(
            text=self.languages[self.current_language]["language_toggle"])
        self.name_label.config(
            text=self.languages[self.current_language]["name"])
        self.title_label.config(
            text=self.languages[self.current_language]["title"])
        self.phone_label.config(
            text=self.languages[self.current_language]["phone"])
        self.age_label.config(
            text=self.languages[self.current_language]["age"])
        self.city_label.config(
            text=self.languages[self.current_language]["city"])
        self.summary_label.config(
            text=self.languages[self.current_language]["summary"])

        # Update other UI elements
        self.photo_button.config(
            text=self.languages[self.current_language]["attach_photo"])
        self.checkbox_photo.config(
            text=self.languages[self.current_language]["show_photo"])
        self.coffee_link.config(
            text=self.languages[self.current_language]["gift_coffee"])

        # Update section buttons
        self.professional_experience_button.config(
            text=self.languages[self.current_language]["add_professional_experience"])
        self.academic_experience_button.config(
            text=self.languages[self.current_language]["add_academic_experience"])
        self.skills_button.config(
            text=self.languages[self.current_language]["add_skills_experience"])
        self.certificates_button.config(
            text=self.languages[self.current_language]["add_certificates_experience"])
    ...
    def add_professional_experience(self):
        ...
        """
        self.add_entry_to_list("professional_experience", "company",
    ...
    def add_academic_experience(self):
        ...
        self.add_entry_to_list(
            "academic_experience", "institution", "degree", "start_date", "end_date")

    def add_skill(self):
        """Agrega una nueva habilidad."""
        new_skill = simpledialog.askstring(
            self.languages[self.current_language]["new_skill"], self.languages[self.current_language]["new_skill"] + ":")
        if new_skill:
    ...
    def create_skill_entry(self, parent, skill, index):
        ...
        entry.bind("<FocusOut>", lambda e: self.on_skill_change(
            index, entry.get()))
        tk.Button(frame, text=self.languages[self.current_language]["remove"],
                  command=lambda: self.remove_skill(index)).pack(side='left', padx=5)
    ...
    def add_certificates_section(self):
        ...
        tk.Label(self.preview_content, text=self.languages[self.current_language]["certificates"], font=(
            'Arial', 16, 'bold')).pack(anchor='w', pady=(10, 0))
        ...
        tk.Button(self.preview_content, text=self.languages[self.current_language]
                  ["add_certificates_experience"], command=self.add_certificate).pack(anchor='w', pady=5)
    ...
    def create_academic_experience_entry(self, parent, exp, index):
        ...
        self.create_labeled_entry(frame, self.languages[self.current_language]["institution"], exp['institution'],
                                  lambda val: self.on_experience_change('academic_experience', index, 'institution', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["degree"], exp['degree'],
                                  lambda val: self.on_experience_change('academic_experience', index, 'degree', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["start_date"], exp['start_date'],
                                  lambda val: self.on_experience_change('academic_experience', index, 'start_date', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["end_date"], exp['end_date'],
                                  lambda val: self.on_experience_change('academic_experience', index, 'end_date', val))
        tk.Button(frame, text=self.languages[self.current_language]["remove"], command=lambda: self.remove_experience(
            'academic_experience', index)).pack(anchor='e')

    def create_certificate_entry(self, parent, cert, index):
        """Creates a certificate entry in the preview."""
        frame = tk.Frame(parent, bd=1, relief='solid', padx=5, pady=5)
        frame.pack(fill='x', pady=5)
        self.create_labeled_entry(frame, self.languages[self.current_language]["name"], cert['name'],
                                  lambda val: self.on_experience_change('certificates', index, 'name', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["institution"], cert['institution'],
                                  lambda val: self.on_experience_change('certificates', index, 'institution', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["date"], cert['date'],
                                  lambda val: self.on_experience_change('certificates', index, 'date', val))
        tk.Button(frame, text=self.languages[self.current_language]["remove"], command=lambda: self.remove_experience(
            'certificates', index)).pack(anchor='e')
    ...
    def add_entry_to_list(self, list_name, *fields):
        ...
        for i, field in enumerate(fields):
            tk.Label(window, text=self.languages[self.current_language][field]).grid(
    ...
    def add_personal_information_section(self):
        ...
        tk.Label(self.preview_content, text=self.languages[self.current_language]["main_title"], font=(
            'Arial', 16, 'bold')).pack(anchor='w')
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["name"], self.name.get(), self.name)
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["title"], self.title.get(), self.title)
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["phone"], self.phone.get(), self.phone)
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["age"], self.age.get(), self.age)
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["city"], self.city.get(), self.city)
        self.create_editable_field(
            self.preview_content, self.languages[self.current_language]["summary"], self.summary.get(), self.summary)

    def add_professional_experience_section(self):
        """Adds the Professional Experience section to the preview."""
        tk.Label(self.preview_content, text=self.languages[self.current_language]["professional_experience"], font=(
            'Arial', 16, 'bold')).pack(anchor='w', pady=(10, 0))
        for index, exp in enumerate(self.professional_experience):
            self.create_professional_experience_entry(
                self.preview_content, exp, index)
        tk.Button(self.preview_content, text=self.languages[self.current_language]
                  ["add_professional_experience"], command=self.add_professional_experience).pack(anchor='w', pady=5)

    def add_academic_experience_section(self):
        """Adds the Academic Experience section to the preview."""
        tk.Label(self.preview_content, text=self.languages[self.current_language]["academic_experience"], font=(
            'Arial', 16, 'bold')).pack(anchor='w', pady=(10, 0))
        for index, exp in enumerate(self.academic_experience):
            self.create_academic_experience_entry(
                self.preview_content, exp, index)
        tk.Button(self.preview_content, text=self.languages[self.current_language]
                  ["add_academic_experience"], command=self.add_academic_experience).pack(anchor='w', pady=5)

    def add_skills_section(self):
        """Adds the Skills section to the preview."""
        tk.Label(self.preview_content, text=self.languages[self.current_language]["skills"], font=(
            'Arial', 16, 'bold')).pack(anchor='w', pady=(10, 0))
        ...
        tk.Button(self.preview_content, text=self.languages[self.current_language]
                  ["add_skills_experience"], command=self.add_skill_from_entry).pack(anchor='w', pady=5)
    ...
    def create_editable_field(self, parent, label_text, value, variable):
        ...
        entry.bind("<FocusOut>", lambda e: self.on_personal_info_change(
            variable, entry.get()))
    ...
    def create_professional_experience_entry(self, parent, exp, index):
        ...
        frame = tk.Frame(parent, bd=1, relief='solid', padx=5, pady=5)
        frame.pack(fill='x', pady=5)
        self.create_labeled_entry(frame, self.languages[self.current_language]["company"], exp['company'], lambda val: self.on_experience_change(
            'professional_experience', index, 'company', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["position"], exp['position'],
                                  lambda val: self.on_experience_change('professional_experience', index, 'position', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["start_date"], exp['start_date'],
                                  lambda val: self.on_experience_change('professional_experience', index, 'start_date', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["end_date"], exp['end_date'],
                                  lambda val: self.on_experience_change('professional_experience', index, 'end_date', val))
        self.create_labeled_entry(frame, self.languages[self.current_language]["description"], exp.get(
            'description', ''), lambda val: self.on_experience_change('professional_experience', index, 'description', val))
        tk.Button(frame, text=self.languages[self.current_language]["remove"], command=lambda: self.remove_experience(
            'professional_experience', index)).pack(anchor='e')
    ...
    def save_data_from_preview(self):
        ...
        if self.is_data_modified:
            try:
                ...
                messagebox.showerror(
                    "Save Error", f"An error occurred while saving: {str(e)}")
        else:
    ...
    def generate_curriculum(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Generate Error", f"An error occurred while generating the CV: {str(e)}")
